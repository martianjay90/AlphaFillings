    ...
    test(`${fixture.name}: ${fixture.kind === 'zip' ? 'XML 해시 검증' : '3회 반복 동일성 + UI 카드 snapshot'}`, async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (guardrail.noneCount > 0) {
          console.log(`[SmokeTest Samsung] ${unavailableReport.message}`)
        }
        ...
        // 1회차: bundle 저장 (스냅샷용) 및 삼성 부채 가드레일 검증
        if (i === 0) {
          firstBundle = bundle
          
          // 삼성 부채 가드레일: 잘못된 작은 값(주석 테이블) 채택 방지
          const latest = firstBundle.statements[0]
          const debt = latest.balance?.interestBearingDebt?.value
          const cash = latest.balance?.cash?.value
          
          expect(debt).toBeDefined()
          expect(cash).toBeDefined()
          
          // 삼성 Q3 2025 기준: debt가 1조 미만이면 비정상으로 간주 (회귀 방지)
          expect(debt!).toBeGreaterThan(1_000_000_000_000) // 1조 미만이면 FAIL
